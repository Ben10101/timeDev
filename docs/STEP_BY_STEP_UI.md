# Guia: Implementando a Geração de Projeto Passo a Passo

Para permitir que o usuário execute cada agente individualmente e valide o resultado de cada etapa, é necessária uma nova interface no frontend. A arquitetura do backend e dos scripts Python foi atualizada para suportar essa funcionalidade através de um novo endpoint: `POST /api/agents/run`.

Este guia mostra como criar um componente React, `PipelineExecutor.jsx`, para gerenciar esse fluxo interativo.

## Visão Geral do Componente

O `PipelineExecutor.jsx` irá:
1.  Manter o estado de qual etapa está ativa.
2.  Armazenar os artefatos gerados (backlog, requisitos, etc.).
3.  Chamar o novo endpoint da API para cada etapa.
4.  Exibir o resultado de cada agente assim que for gerado.
5.  Permitir que o usuário avance para a próxima etapa.

## Exemplo de Código: `PipelineExecutor.jsx`

Você pode criar este novo componente em `frontend/src/components/PipelineExecutor.jsx`.
    ...
    try {
      ...
      stage.dependsOn.forEach(depKey => {
        if (depKey === 'developer_output') {
            // O agente developer retorna um objeto, passamos só a parte 'code'
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      {currentStep === PIPELINE_STAGES.length && (
        <div className="mt-8 text-center p-8 bg-green-100 border border-green-300 rounded-lg">
            ...
            {/* Aqui você pode adicionar um botão para baixar o projeto completo */}
        </div>
      )}
    </div>
  );
...
1.  **Instale as dependências do frontend**:
    ...
    ```
2.  **Modifique sua página principal** (provavelmente `HomePage.jsx` ou `App.jsx`) para que, após o usuário inserir a ideia, em vez de navegar para a antiga página de resultados, ele renderize este novo componente `PipelineExecutor`, passando a `idea` como uma prop.

Este novo fluxo oferece a interatividade que você solicitou, permitindo uma validação passo a passo e uma compreensão muito maior do que cada agente da sua fábrica está produzindo.