...
import type { LoginSessionRequest, LoginSessionResponse } from '../../../../../packages/shared/src/contracts/auth-login.ts';
import { createLoginSession, fetchLoginSessionItems } from './service';

const initialForm: LoginSessionRequest = {
  email: '',
  password: '',
};

export function LoginPage() {
  const [items, setItems] = useState<LoginSessionResponse[]>([]);
  const [form, setForm] = useState<LoginSessionRequest>(initialForm);
  const [feedback, setFeedback] = useState('');
  const [errorMessage, setErrorMessage] = useState('');

  useEffect(() => {
    fetchLoginSessionItems().then(setItems).catch(() => setItems([]));
  }, []);
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto', display: 'grid', gap: 32, gridTemplateColumns: 'minmax(0, 1.05fr) minmax(320px, 0.95fr)' }}>
        <div style={{ display: 'grid', gap: 18, alignContent: 'start', paddingTop: 32 }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            <h2 style={{ margin: 0, fontSize: 44, lineHeight: 1.05, color: '#0f172a' }}>Acesse sua Conta</h2>
            <p style={{ margin: 0, fontSize: 18, lineHeight: 1.6, color: '#475569', maxWidth: 560 }}>Faça login com seu e-mail e senha.</p>
          </div>
          <div style={{ display: 'grid', gap: 14, padding: 24, borderRadius: 24, background: 'linear-gradient(135deg, #0f766e, #115e59)', color: '#f8fafc', boxShadow: '0 24px 60px rgba(15, 118, 110, 0.22)' }}>
            <strong style={{ fontSize: 16 }}>Bem-vindo!</strong>
            <p style={{ margin: 0, lineHeight: 1.6, color: 'rgba(248, 250, 252, 0.88)' }}>Clique para acessar a plataforma.</p>
            <div style={{ display: 'grid', gap: 10 }}>
              <div style={{ padding: '14px 16px', borderRadius: 18, background: 'rgba(255,255,255,0.1)' }}>E-mail válido</div>
              <div style={{ padding: '14px 16px', borderRadius: 18, background: 'rgba(255,255,255,0.1)' }}>Senha segura</div>
            </div>
          </div>
        </div>
        ...
        <label style={{ display: 'grid', gap: 6 }}>
          ...
          <div style={{ padding: 24, borderRadius: 24, background: '#ffffff', border: '1px solid #e2e8f0', boxShadow: '0 16px 40px rgba(15, 23, 42, 0.06)' }}>
            <h3 style={{ marginTop: 0, marginBottom: 12, color: '#0f172a' }}>Ultimos registros</h3>
            {items.length ? (
              ...
              <p style={{ margin: 0, color: '#64748b' }}>Nenhum registro processado ainda.</p>
            )}
          </div>
        </div>
      </div>
