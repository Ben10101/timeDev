...
import type { AccountRegistrationRequest, AccountRegistrationResponse } from '../../../../../packages/shared/src/contracts/auth-register.ts';
import { createAccountRegistration, fetchAccountRegistrationItems } from './service';
...
export function RegisterPage() {
  const [items, setItems] = useState<AccountRegistrationResponse[]>([]);
  const [form, setForm] = useState<AccountRegistrationRequest>(initialForm);
  const [feedback, setFeedback] = useState('');
  const [errorMessage, setErrorMessage] = useState('');

  useEffect(() => {
    fetchAccountRegistrationItems().then(setItems).catch(() => setItems([]));
  }, []);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await createAccountRegistration({
      email: form.email,
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto', display: 'grid', gap: 32, gridTemplateColumns: 'minmax(0, 1.05fr) minmax(320px, 0.95fr)' }}>
        <div style={{ display: 'grid', gap: 18, alignContent: 'start', paddingTop: 32 }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            <h2 style={{ margin: 0, fontSize: 44, lineHeight: 1.05, color: '#0f172a' }}>Crie sua Conta</h2>
            <p style={{ margin: 0, fontSize: 18, lineHeight: 1.6, color: '#475569', maxWidth: 560 }}>Acesse todos os cursos com seu e-mail e senha.</p>
          </div>
          <div style={{ display: 'grid', gap: 14, padding: 24, borderRadius: 24, background: 'linear-gradient(135deg, #0f766e, #115e59)', color: '#f8fafc', boxShadow: '0 24px 60px rgba(15, 118, 110, 0.22)' }}>
            ...
            <p style={{ margin: 0, lineHeight: 1.6, color: 'rgba(248, 250, 252, 0.88)' }}>Registre-se para acessar uma variedade de cursos e recursos de aprendizado.</p>
        ...
        <label style={{ display: 'grid', gap: 6 }}>
          ...
          <div style={{ padding: 24, borderRadius: 24, background: '#ffffff', border: '1px solid #e2e8f0', boxShadow: '0 16px 40px rgba(15, 23, 42, 0.06)' }}>
            <h3 style={{ marginTop: 0, marginBottom: 12, color: '#0f172a' }}>Informações</h3>
            {items.length ? (
              ...
              <p style={{ margin: 0, color: '#64748b' }}>Não há informações para exibir.</p>
            )}
          </div>
        </div>
      </div>
