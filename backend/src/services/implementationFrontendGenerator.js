...
export function buildModernFrontendFeatureFiles(task, technicalSpec, { sharedImportPath, uiImportPath, domainTemplate }) {
  ...
  const componentMap =
    screenSpec.componentMap && typeof screenSpec.componentMap === 'object'
      ...
      : technicalSpec.frontend?.componentMap && typeof technicalSpec.frontend.componentMap === 'object'
        ...
        : {};
  const operationHeavyArchetypes = new Set([
    'operations-queue',
    'approval-flow',
    'review-queue',
    'operational-tool',
  ...
  const isOperationFirstLayout =
    operationHeavyArchetypes.has(String(pageArchetype || '').trim().toLowerCase()) ||
    ['queueRail', 'approvalSteps', 'evidenceRail'].includes(String(componentMap.recordsLead || '').trim()) ||
    ['linear-queue', 'evidence-trail', 'review-lane'].includes(String(fallbackPattern || '').trim());
  ...
  const tertiaryField = fields.find((field) => ![previewField?.name, secondaryField?.name, 'password'].includes(field.name)) || secondaryField;
  ...
  const operationalGuidance = JSON.stringify(
    (
      technicalSpec.businessRules?.slice(0, 3) ||
      technicalSpec.qaScenarios?.map((scenario) => scenario.message).slice(0, 3) ||
      ['Registre apenas sinais que mudam a leitura operacional do caso.']
    ).filter(Boolean)
  );
...
`;
  ...
  const operationFirstPageContent = `import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { zodResolver } from '@hookform/resolvers/zod';
import { useForm } from 'react-hook-form';
import type { ${technicalSpec.shared.requestContractName}, ${technicalSpec.shared.responseContractName} } from '${sharedImportPath}';
import { FieldGroup, PrimaryButton, SurfaceCard, MetricRow, Badge, inputStyle } from '${uiImportPath}';
import { ${schemaName}, type ${formValuesType} } from './schema';
import { ${queryKeyName}, create${entityName}, fetch${entityName}Items } from './service';
...
const guidanceItems = ${operationalGuidance};
...
function badgeColors(accent = '${escapeTemplate((technicalSpec.frontend.accent || domainTemplate?.accent || 'teal'))}') {
  if (accent === 'amber') {
    ...
    <section style={{ display: 'grid', gap: 18 }}>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div style={{ display: 'grid', gap: 8, maxWidth: 820 }}>
            <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#64748b' }}>
              ${JSON.stringify(technicalSpec.frontend.heroEyebrow || technicalSpec.frontend.navigationLabel || technicalSpec.entityName)}
            </div>
            <h1 style={{ margin: 0, fontSize: 34, lineHeight: 1.02, letterSpacing: '-0.04em', color: '#0f172a' }}>
              ${JSON.stringify(technicalSpec.frontend.heroTitle || technicalSpec.frontend.pageTitle || technicalSpec.entityName)}
            </h1>
            <p style={{ margin: 0, color: '#475569', lineHeight: 1.75, fontSize: 15 }}>
              ${JSON.stringify(technicalSpec.frontend.heroDescription || technicalSpec.frontend.pageDescription || technicalSpec.summary)}
            </p>
          </div>
          <div style={{ display: 'grid', gap: 10, justifyItems: 'end' }}>
            ...
            >
              ${JSON.stringify(pageArchetype || 'operation')}
            </span>
            <div style={{ color: '#64748b', fontSize: 13 }}>
              ${JSON.stringify(technicalSpec.domain.submitLabel)} como proxima acao principal
            </div>
          </div>
        </div>
        ...
        <MetricRow
          items={[
            ...
            { label: 'Foco', value: ${JSON.stringify(technicalSpec.frontend.formCardTitle || technicalSpec.frontend.pageTitle || technicalSpec.entityName)} },
          ]}
        />
      </section>
      ...
      >
        <div style={{ display: 'grid', gap: 18 }}>
          <SurfaceCard
            title="${escapeTemplate(technicalSpec.frontend.recordsTitle || domainTemplate?.recordsTitle || 'Painel operacional')}"
            description="${escapeTemplate(technicalSpec.frontend.recordsEmptyState || domainTemplate?.recordsEmptyState || 'Acompanhe aqui os registros que ajudam a leitura operacional desta frente.')}"
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              {isLoading ? (
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'start', flexWrap: 'wrap' }}>
                      <div style={{ display: 'grid', gap: 6 }}>
                        <strong style={{ color: '#0f172a', fontSize: 16 }}>
                          {String(item.${previewField?.name || 'id'} || item.id)}
                        </strong>
                        <div style={{ color: '#475569', fontSize: 14, lineHeight: 1.7 }}>
                          {String(item.${secondaryField?.name || 'status'} || 'Registro pronto para leitura operacional.')}
                        </div>
                      </div>
                    ...
                    >
                      <div style={{ padding: '12px 14px', borderRadius: 14, background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ marginTop: 6, color: '#0f172a', fontSize: 14, lineHeight: 1.6 }}>
                          {String(item.${tertiaryField?.name || previewField?.name || 'id'} || item.id)}
                        </div>
                      </div>
                ...
                <div style={{ padding: 30, borderRadius: 18, background: '#f8fafc', border: '1px dashed #cbd5e1', display: 'grid', gap: 8, textAlign: 'center' }}>
                  ...
                  <strong style={{ color: '#0f172a', fontSize: 16 }}>${JSON.stringify(technicalSpec.frontend.recordsTitle || domainTemplate?.recordsTitle || 'Painel operacional')}</strong>
                  <p style={{ margin: 0, color: '#64748b', lineHeight: 1.7 }}>
                    ${JSON.stringify(technicalSpec.frontend.recordsEmptyState || domainTemplate?.recordsEmptyState || 'A primeira entrada deve aparecer aqui para iniciar a trilha de acompanhamento.')}
                  </p>
                </div>
              )}
            </div>
        ...
        <div style={{ display: 'grid', gap: 18 }}>
          <SurfaceCard
            title="${escapeTemplate(technicalSpec.frontend.formCardTitle || technicalSpec.frontend.pageTitle || technicalSpec.entityName)}"
            description="${escapeTemplate(technicalSpec.frontend.formCardDescription || technicalSpec.frontend.pageDescription || technicalSpec.summary)}"
            background="#ffffff"
          >
            <form onSubmit={handleSubmit((values) => mutation.mutateAsync({
${payloadObject}
...
`;
  ...
  const pageContent = normalizeSharedUiImportPath(hasHealthyAutonomousPageTemplate
    ? normalizedAutonomousPageTemplate
    : isOperationFirstLayout
      ? operationFirstPageContent
