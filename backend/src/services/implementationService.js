    ...
    if (normalizedWrittenFiles.length) {
        await prisma.generatedFile.deleteMany({
            where: {
                ...
                filePath: { in: normalizedWrittenFiles.map((file) => file.relativePath.replace(/\\/g, '/')),
                },
            },
        });
...
async function hydrateTechnicalSpecWithWorkspaceImplementation(technicalSpec, generatedAppRoot) {
    ...
    return {
        ...
        frontend: {
            ...
            autonomousPageTsxTemplate: technicalSpec.frontend ?.autonomousPageTsxTemplate || workspaceFrontendFiles.pageTsxTemplate || null,
            autonomousServiceTsTemplate: technicalSpec.frontend ?.autonomousServiceTsTemplate || workspaceFrontendFiles.serviceTsTemplate || null,
            autonomousIndexTsTemplate: technicalSpec.frontend ?.autonomousIndexTsTemplate || workspaceFrontendFiles.indexTsTemplate || null,
            autonomousFileSources: Object.keys(nextFrontendSources).length ? nextFrontendSources : null,
        },
        backend: {
            ...technicalSpec.backend,
            autonomousServiceTsTemplate: technicalSpec.backend ?.autonomousServiceTsTemplate || workspaceBackendFiles.serviceTsTemplate || null,
            autonomousRouterTsTemplate: technicalSpec.backend ?.autonomousRouterTsTemplate || workspaceBackendFiles.routerTsTemplate || null,
            autonomousIndexTsTemplate: technicalSpec.backend ?.autonomousIndexTsTemplate || workspaceBackendFiles.indexTsTemplate || null,
...
function buildUiGenerationContext(task, technicalSpec, repairContext = null) {
    ...
    return {
        ...
        uiRole: screenTemplate === 'settings' ?
            'configuracao' : screenTemplate === 'dashboard' ?
            'acompanhamento' : screenTemplate === 'workspace' ?
            'operacao guiada' : screenTemplate === 'wizard' ?
            'progressao' : 'operacao',
...
function buildAutonomousGenerationSummary(technicalSpecContent = {}) {
    ...
    return {
        ...
        autonomyPercent,
        variationProfile: materialization.variationProfile ||
            technicalSpecContent ?.frontend ?.autonomousVariationProfile ||
            technicalSpecContent ?.autonomousExecution ?.variationProfile ||
            null,
        compositionSignature: technicalSpecContent ?.frontend ?.autonomousCompositionSignature ||
...
function resolveRepairStyle({ technicalSpec, findings, specialistFindings, validationFailures }) {
    ...
    const frontendOnlyReview = !validationFailures.length &&
        allSignals.length > 0 &&
...
async function buildRepairContext({ reviewReport, specialistReviewReport, validationSummary, attemptNumber, technicalSpec, projectId, forcedDirective = null }) {
    ...
    const baseContext = {
        ...
        validationFailures,
        generationSource: technicalSpec ?.autonomousMaterialization ?.generationSource ||
            technicalSpec ?.frontend ?.autonomousGenerationSource ||
            technicalSpec ?.autonomousExecution ?.generationSource ||
            'unknown',
        materialization: technicalSpec ?.autonomousMaterialization || null,
        debugDiagnosis,
        repairScope,
...
function buildImplementationQualitySummary({ task, implementation, reviewArtifact, specialistReviewArtifact, buildReportArtifact, testReportArtifact, lintReportArtifact }) {
    ...
    const repairBehavior = {
        ...
        adherencePercent: scopeAssessmentContent ?.scopeAssessment ?.adherencePercent ??
            executionStateContent ?.repairScopeAssessment ?.adherencePercent ??
            null,
        outsideWriteSetCount: scopeAssessmentContent ?.scopeAssessment ?.outsideWriteSet ?.length ??
        ...
        nextRepairStyle: enforcementContent ?.enforcementDirective ?.nextRepairStyle ||
            executionStateContent ?.enforcementDirective ?.nextRepairStyle ||
            null,
        nextExecutor: enforcementContent ?.enforcementDirective ?.nextExecutor ||
    ...
    return {
        ...
        validationScore,
        screenTemplate: reviewContent ?.structured ?.classification ?.screenTemplate ||
            technicalSpecContent ?.architecture ?.screenTemplate ||
            technicalSpecContent ?.structured ?.classification ?.screenTemplate ||
            null,
    };
...
function classifyLaneRisk(lane, qualitySummary = {}) {
    const laneSignals =
        lane === 'backend' ? ['buildStatus', 'specialistArchitectureScore'] :
        lane === 'frontend' ? ['lintStatus', 'uxScore'] : ['testStatus', 'validationScore'];
...
function summarizeLatestFailure(implementation) {
    ...
    const validationEvidence =
        testArtifact ?.status !== 'completed' ? { scriptName: testArtifact ?.scriptName || 'test', ...testArtifact } :
        lintArtifact ?.status !== 'completed' ? { scriptName: lintArtifact ?.scriptName || 'lint', ...lintArtifact } :
        failedBuildReport ? { scriptName: failedBuildReport.scriptName || 'build', ...failedBuildReport } :
        null;
...
function buildOperationalFocusSummary(implementations = []) {
    ...
    return {
        ...
        latestFailedTask: {
            ...
            validationStatus: latestFailure.validationStatus,
        },
    };
}
    ...
    return {
        ...
        summary: {
            ...
            recommendation: risk.level === 'high' ?
                'Revisar manualmente o diff antes de considerar a integração encerrada.' : risk.level === 'medium' ?
                'Revisar contratos, rotas e regressões principais antes de promover a mudança.' : 'Mudança localizada; a revisão pode focar nos fluxos principais e consistência final.',
        },
        filesByArea: grouped,
...
async function buildProjectMemorySnapshot(task, generatedApp) {
    ...
    return {
        ...
        summary: {
            ...
            preferredScreenTemplate: Object.entries(screenTemplates).sort((a, b) => b[1] - a[1])[0] ?.[0] || 'crud',
        },
        featurePatterns,
...
function buildBlueprintSeedBrief(featureKey) {
    const briefs = {
        'support-performance-dashboard': {
            ...
            description: 'Painel gerencial para acompanhar chamados, gargalos de atendimento, status, categorias e prioridades da operacao.',
        },
        'support-ticket-attachments': {
            title: 'Anexos e evidencias para chamados de suporte',
            description: 'Fluxo para registrar comprovantes, documentos, imagens e arquivos associados ao ticket para acelerar a triagem.',
        },
        'ticket-notification-preferences': {
            title: 'Preferencias de notificacao por e-mail para chamados',
            description: 'Tela para configurar e-mail principal e alertas de atualizacao do chamado sem depender do portal.',
        },
        'access-control-roles': {
            title: 'Perfis de acesso e permissoes da operacao',
            description: 'Governanca de acesso por perfil, funcao e escopo para controlar quem pode atender, revisar e administrar a operacao.',
        },
        'course-catalog': {
            title: 'Criar curso com nome, descricao, categoria e preco',
            description: 'Fluxo comercial e editorial para cadastrar cursos e preparar a oferta para a vitrine.',
        },
        'course-modules': {
            title: 'Organizar modulos do curso',
            description: 'Estruturar o curso em modulos com sequencia logica e contexto pedagogico claro.',
        },
        'course-lessons': {
            title: 'Cadastrar aulas do curso',
            description: 'Associar aulas aos modulos e escolher tipo de midia para o conteudo.',
        },
        'lesson-materials': {
            title: 'Materiais complementares para aulas',
            description: 'Biblioteca de materiais, anexos e arquivos de apoio vinculados as aulas.',
        },
        'course-pricing': {
            title: 'Preco e configuracao comercial do curso',
            description: 'Definir valor, condicoes comerciais e posicionamento para venda do curso.',
        },
        'course-search': {
            title: 'Busca e descoberta de cursos por categoria',
            description: 'Experiencia de pesquisa por palavra-chave, filtros e catalogo para facilitar descoberta.',
        },
        'course-enrollment': {
            title: 'Matricular aluno em curso',
            description: 'Fluxo de inscricao e liberacao de acesso do aluno aos cursos corretos.',
        },
        'course-player': {
            title: 'Player de curso com video, audio e progresso',
            description: 'Tela para consumir aulas, acompanhar progresso e retomar conteudo.',
        },
    };
...
function applyAutonomousImplementationDraft(technicalSpec, autonomousDraft) {
    ...
    return {
        ...
        frontend: {
            ...
            autonomousPageTsxTemplate: preservedPageTemplate ||
            ...
            componentMap: frontend.componentMap && typeof frontend.componentMap === 'object' ? {
                ...(technicalSpec.frontend ?.componentMap || {}),
            ...
            autonomousGenerationSource: autonomousDraft.generationSource ||
                materialization ?.generationSource ||
                technicalSpec.frontend ?.autonomousGenerationSource ||
                null,
            autonomousVariationProfile: autonomousDraft.variationProfile ||
                materialization ?.variationProfile ||
                technicalSpec.frontend ?.autonomousVariationProfile ||
                null,
            autonomousCompositionSignature: autonomousDraft.compositionSignature ||
...
function buildAutonomousCurrentImplementationContext(technicalSpec = {}) {
    ...
    return {
        featureKey: technicalSpec ?.featureKey || null,
        generationSource: autonomousMaterialization ?.generationSource ||
            frontend ?.autonomousGenerationSource ||
            autonomousExecution ?.generationSource ||
            null,
        variationProfile: autonomousMaterialization ?.variationProfile ||
            frontend ?.autonomousVariationProfile ||
            autonomousExecution ?.variationProfile ||
            null,
        compositionSignature: frontend ?.autonomousCompositionSignature ||
...
async function loadProjectCoherenceContracts(projectId) {
    const systemTasks = await prisma.task.findMany({
        where: {
            projectId,
            title: { in: ['[SYSTEM] Backlog Master', '[SYSTEM] Architecture Master'],
            },
        },
        select: {
            title: true,
...
function runUiProductPolishPass(draft = {}, context = {}) {
    ...
    if (!polished.profileSummaryTitle ||
    ...
    if (!polished.formCardDescription ||
...
function inferPermissions(sourceText) {
    ...
    return {
        ...
        actor: /\bassistente de recepcao\b|\brecepcionista\b/.test(normalized) ?
            'receptionist' : /\baluno\b/.test(normalized) ?
            'student' : /\binfoprodutor\b/.test(normalized) ?
            'creator' : 'user',
        scope: /\badmin\b|\badministrador\b/.test(normalized) ?
            'admin' : /\bvisita\b|\bvisitas\b|\brecepcao\b|\boperacional\b/.test(normalized) ?
            'operations' : 'self_service',
    };
}
...
function sanitizeArchitectureSummaryItems(items = [], { kind = 'generic', limit = 5 } = {}) {
    ...
    const kindPatterns =
        kind === 'stack' ? [/\bfrontend\b/i, /\bbackend\b/i, /\breact\b/i, /\bexpress\b/i, /\bvite\b/i, /\bprisma\b/i, /\bpostgres/i, /\btypescript\b/i] : [/\bmodulo\b/i, /\bmodulos\b/i, /\bresponsab/i, /\bvisita\b/i, /\bresponsavel\b/i, /\boperac/i, /\broute\b/i, /\brota\b/i, /\bcontrato\b/i];
