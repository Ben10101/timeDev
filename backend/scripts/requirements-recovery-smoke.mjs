...
const state = {
  project: { id: 77, uuid: 'project-req-1' },
  user: { id: 5, uuid: 'user-1' },
  task: {
    id: 501,
    uuid: 'task-req-1',
    title: 'Como coordenador, eu quero criar um evento, para iniciar o planejamento.',
    taskType: 'story',
    status: 'in_progress',
    assigneeType: 'agent',
    assigneeUserId: null,
    ...
    checklistItems: [],
    agentRuns: [],
    creator: { uuid: 'creator-1', name: 'Creator', email: 'creator@example.com' },
    assigneeUser: null,
    reporterUser: null,
...
try {
  ...
  await finishAgentRun(createdRun.id, {
    status: 'failed',
    errorMessage: 'Falha simulada do requirements_analyst.',
  });
  ...
  assert(finalRun?.status === 'failed', 'A nova run deveria terminar em failed.');
  assert(state.task.status === 'backlog', 'A task deveria voltar para backlog apos falha.');
  assert(state.task.startedAt === null, 'A task nao deveria manter startedAt apos recovery.');
  assert(
    state.statusHistory.some((item) => String(item.note || '').includes('Falha ao executar requirements_analyst')),
    'O historico deveria registrar a devolucao da task apos falha.'
  );

  console.log('requirements-recovery-smoke: ok');
} finally {
  restoreMocks();
}
